...
%% \item ({\bf 3 Points}) What is {\em meta-data}?  ow does the Java
...
\item ({\bf 3 Points}) The Java database connectivity (JDBC) interface furnishes the ability to create and submit a {\em
  prepared statement}.  Furthermore, JDBC also enables a program to execute a string representation of a SQL {\bf
  select} and receive a {\em result set}.  Using source code statements when appropriate, define the terms prepared
  statement and result set. What are the security implications associated with using these two methods to interact with
...
\item ({\bf 10 Points}) You must consider security and performance
  ...
  \begin{enumerate}
    ...
    \item ({\bf 3 Points}) What is {\em authentication}?  {\em Authorization}?  How does the structured query language
      support these two types of security features?
...
\item ({\bf 10 Points}) The eXtensible Markup Language (XML) is a popular data interchange format.  Using
  Figure~\ref{figure:xml} as appropriate, answer the following questions about XML.
  ...
  \item ({\bf 2 Points}) Many authors suggest that XML is a very verbose data storage format.  Do you agree with this
    statement?  Why or why not? Using the XML file in Figure~\ref{figure:xml}, you should provide evidence to support
    your response.
...
\item ({\bf 10 Points}) Answer the following questions about database applications and their use during the data mining
  ...
  \item ({\bf 2 Points}) In constrast to the traditional row-oriented relational database, it is also possible to
    implement and use a column-oriented database. For each category, please describe a query pattern that, when run on a
    column-oriented database, will result in (i) exceptional performance and (ii) poor performance. 

  \item ({\bf 2 Points}) Data warehouses often contain large data sets with a mixture of {\em categorical} and {\em
    numerical} attributes.  As part of your response to this question, please define and give an example of these two
    types of attributes.

  \item ({\bf 4 Points}) During the algorithm training and evaluation phases of data mining, it is common to use {\em
    k-fold cross validation}. After explaining this process in detail---with both diagrams and natural langauge
    sentences---you should justify its use during the process of data mining.
...
\item ({\bf 10 Points}) Answer the following questions about the use of machine learning algorithms during the processes
  of data mining. Your response to this question should be informed by the attribute definitions in
  Figure~\ref{fig:meaning} and the trees in Figure~\ref{fig:trees}.

  \begin{enumerate}

  \item ({\bf 2 Points}) Machine learning algorithms generally focus on prediction tasks in either a {\em
  ...
  \item ({\bf 4 Points}) Figure~\ref{fig:trees} gives an example of two regression trees that were constructed by the
    ...
    \begin{enumerate}
      ...
      \item An inspection of the trees reveals that they do not use all of the attributes described in
        Figure~\ref{fig:meaning}---can you explain why this may be the case?
      ...
      \item Give a detailed description of a CPU configuration that yields high values for the ``perf'' attribute. How
        does a tree help you to find this CPU design?

    \end{enumerate}

  \item ({\bf 2 Points}) How would you use the regression tree in Figure~\ref{fig:trees} to perform prediction?  Your
    response to this question should furnish a concrete example that explains how to predict with one of the trees that
    describe CPU performance.

  \item ({\bf 2 Points}) There are algorithms that use either {\em global} or {\em local} search heuristics to construct
    a classification and regression tree. What are the differences between these two methods? Which of the trees in
    Figure~\ref{fig:trees} was produced by a global, evolutionary search method? What characteristic(s) suggest that
    you are correct?
