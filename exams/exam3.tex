...
\item ({\bf 10 Points}) Answer the following questions about database applications and their use during the pdata mining
  ...
  \item ({\bf 2 Points}) In constrast to the traditional row-oriented relational database, it is also possible to
    implement and use a column-oriented databases. For each category, please describe a query pattern that, when run on
    a column-oriented database, will result in (i) exceptional performance and (ii) poor performance. 
  ...
  \item ({\bf 4 Points}) Figure~\ref{fig:trees} gives an example of two regression trees that were constructed by the
    ...
    \begin{enumerate}
      ...
      \item Give a detailed description of a CPU configuration that yields high values for the ``perf'' attribute. How
        does the tree help you to find this CPU design?
